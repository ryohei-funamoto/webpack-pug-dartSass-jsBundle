...
const metaData = require("./src/data/site.json");
...
const htmlGlobPlugins = (entries) => {
    // Globの条件に合致したPugファイルをHTMLファイルに変換する
    return Object.keys(entries).map(
        (key) => new HtmlWebpackPlugin({
            filename: `${key}.html`, // 出力ファイル名
            template: `${filePath.pug}${key}.pug`, // 読み込むPugファイルのパス
        ...
        rules: [ // 各ローダーの設定
            {
                ...
                use: [
                    "html-loader",
                    {
                        ...
                        options: {
                            pretty: true,
                            data: { // site.jsonのglobalとlocalをそれぞれの変数として設定する
                                globalConf: metaData["global"],
                                localConf: metaData["local"],
                            },
                            basePath: path.resolve(__dirname, "src/pug") // 基準となるパスを設定する
                        }
                    },
                ],
            },
